...
- **Unit/integration:** Vitest with jsdom. Test files live alongside source as `*.test.ts` / `*.test.tsx`.
- **Storybook component tests:** `*.stories.tsx` files, run in Playwright browser mode via `@storybook/addon-vitest`. Storybook config: `.storybook/`.
- `bun run test` runs both `unit` and `storybook` Vitest projects.
- Driver-contract tests (e.g. `push-store.driver-contract.test.ts`) validate that the SQLite abstraction behaves identically under both drivers.
- Bun-runtime tests (`*.bun-runtime.test.ts`) are skipped on Node; Bun-branch tests (`*.bun-branch.test.ts`) test the Bun code path via mocking.
- The Frigate client has a `mock-client.ts` used by tests; set `FRIGATE_MOCK=true` in the test env to activate it.
