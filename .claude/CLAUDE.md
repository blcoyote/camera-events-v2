...
| **Implement**   | Test-Driven Development, Systematic Debugging, Mutation Testing, Browser Testing, Performance Benchmark, CI Debugging, TanStack Start Boundaries                                       | Build with TDD, debug issues, validate quality, measure performance |
...
1. **[Context Loading Protocol](skills/context-loading-protocol/SKILL.md)** - decides _what_ to load and _when_, using task classification, phased loading, and measured token budgets
2. **[Context Summarization](skills/context-summarization/SKILL.md)** - decides _when_ to compress and _how_, using LSTM-inspired gates, utilization triggers, and structured summaries written to `memory/`
