/**
 * Vendored third-party file: video-rtc.js
 *
 * Source:  https://raw.githubusercontent.com/AlexxIT/go2rtc/v1.9.14/www/video-rtc.js
 * Project: go2rtc (https://github.com/AlexxIT/go2rtc)
 * Tag:     v1.9.14 (identical byte-for-byte to the `master` branch HEAD at
 *          retrieval time; the file's own internal version comment reads
 *          "VideoRTC v1.6.0" — go2rtc versions this file independently of
 *          the go2rtc release tag)
 * Retrieved: 2026-07-19
 * License: MIT (go2rtc is MIT-licensed; original author AlexxIT
 *          <https://github.com/AlexxIT>). Copyright (c) go2rtc contributors.
 *
 * This file is vendored VERBATIM below this header comment block — no lines
 * of the original source were changed. It already exports `VideoRTC` as an
 * ES module class and does not call `customElements.define(...)` itself, so
 * no edit was needed to use it as a module: the consumer (see
 * `#/features/live/components/LiveMsePlayer.tsx`) imports `VideoRTC` and
 * registers the `video-stream` custom element itself.
 *
 * KNOWN GAP (checked go2rtc tags v1.9.0 through v1.9.14 and `master` as of
 * 2026-07-19): this file references `ManagedMediaSource` and sets
 * `disableRemotePlayback = true` for iOS Safari 17+, but it does NOT listen
 * for the ManagedMediaSource `startstreaming`/`endstreaming` flow-control
 * events anywhere. Frigate's own custom MSE player
 * (web/src/components/player/MsePlayer.tsx) has the same gap. Flagged here
 * rather than silently patched — see the implementation report for details.
 */
