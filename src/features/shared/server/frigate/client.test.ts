...
describe('retainEvent', () => {
  ...
  it('makes POST request to correct retain URL', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      `${FRIGATE_URL}/api/events/1678886400.123-abc/retain`,
  ...
  it('returns { ok: false } with status on HTTP 404', async () => {
    globalThis.fetch = mockFetchJson({ detail: 'not found' }, 404)
    const { retainEvent } = await import('./client')
    const result = await retainEvent('bad-id')
  ...
  it('does not populate the cache', async () => {
    globalThis.fetch = mockFetchJson({ success: true })
    const { retainEvent, getEvent } = await import('./client')
    await retainEvent('1678886400.123-abc')

    globalThis.fetch = mockFetchJson(MOCK_EVENT)
    await getEvent('1678886400.123-abc')
...
describe('unretainEvent', () => {
  ...
  it('makes DELETE request to correct retain URL', async () => {
    globalThis.fetch = mockFetchJson({ success: true })
  ...
  it('returns { ok: false } with status on HTTP 404', async () => {
    globalThis.fetch = mockFetchJson({ detail: 'not found' }, 404)
    const { unretainEvent } = await import('./client')
    const result = await unretainEvent('bad-id')
  ...
  it('does not populate the cache', async () => {
    globalThis.fetch = mockFetchJson({ success: true })
    const { unretainEvent, getEvent } = await import('./client')
    await unretainEvent('1678886400.123-abc')
