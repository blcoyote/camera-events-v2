...
describe('EventBatcher', () => {
  ...
  it('flushes a single event after the window expires', () => {
    const flush = vi.fn()
    batcher = new EventBatcher(flush, 10_000)
  ...
  it('batches multiple events from the same camera', () => {
    ...
    vi.advanceTimersByTime(4_000) // total 10s from first event
    expect(flush).toHaveBeenCalledOnce()
  ...
  it('maintains independent windows per camera', () => {
    ...
    vi.advanceTimersByTime(5_000) // front_porch fires at 10s
    expect(flush).toHaveBeenCalledOnce()
    expect(flush.mock.calls[0][0]).toBe('front_porch')

    vi.advanceTimersByTime(5_000) // driveway fires at 15s
